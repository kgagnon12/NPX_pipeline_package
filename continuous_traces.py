...
# kim hardcode 'num_channels' to avoid settings.xml parse error 20201202
def get_surface_channel_spikeband(path,start=10.,end=18.,num_channels=384,sampling_rate=30000,plot=False,filter_size=2,sigma=1.,filter=False,probemap=None):
...
def get_surface_channel_gamma(path,start=2.,end=8.,sampling_rate=2500,plot=False):
	mm = np.memmap(path, dtype=np.int16, mode='r')
...
def get_surface_channel_freq(path,frequency_range=[1,5],num_channels=384,start=2.,end=8.,sampling_rate=2500,filter_size=2,sigma=2.,plot=False,filter=False,probemap=None):
	mm = np.memmap(path, dtype=np.int16, mode='r')
	#num_channels =  get_channel_count(os.path.dirname(path),from_channel_map=False)
	chunk = get_chunk(mm,start,end,num_channels,sampling_rate)
	if probemap is not None:
...
def get_probe_freq(path,frequency_range=[1,5],num_channels=384,start=2.,end=10.,sampling_rate=2500.,filter=False,probemap=None):
	mm = np.memmap(path, dtype=np.int16, mode='r')
	#num_channels = get_channel_count(os.path.dirname(path),from_channel_map=False)
	chunk = get_chunk(mm,start,end,num_channels,sampling_rate)
	if probemap is not None:
